...
class MSinGAN(BaseGAN):
    """MSinGAN.

    A modified version for SinGAN, which supports training with multiple images.
    ...
    def __init__(self,
        ...
        self.noise_weights_dict = {} 
        self.fixed_noises = [] # 当前使用的 fixed_noises
        self.fixed_noises_dict = {}  # change fixed_noise to dict
        self.real = []
        self.reals_dict = {}  # 改原本的list 为dict， 为训练集中的每张图都准备一个真实图的金字塔
        self.input_index = '0' # 用于指示当前训练图片的序号
    ...
    def construct_fixed_noises(self):
        """Construct the fixed noises list used in MSinGAN."""
        for i, real in enumerate(self.reals):
            ...
            if i == 0:
                ...
                self.fixed_noises.append(noise)
        self.fixed_noises_dict[self.input_index] = self.fixed_noises
    ...
    def forward(self,
        ...
        input_index = gen_kwargs.pop('input_index', 0)
    ...
    def train_step(self, data: dict,
        ...
        if curr_iter % (self.iters_per_scale * self.discriminator_steps) == 0:
            ...
            _warning_msg = ('MSinGAN do set batch size as 1 during training '
        ...
        inputs_dict, data_samples, self.input_index = \
            data['inputs'], data['data_samples'], data['input_index'] # dataset 添加一个输出 input_index
        
        # TODO:
        if isinstance(self.input_index, (list, tuple)):
            assert len(self.input_index) == 1
            self.input_index = self.input_index[0]
        # setup fixed noises and reals pyramid
        if self.reals_dict.get(self.input_index, False):
            self.reals = self.reals_dict[self.input_index]
            self.fixed_noises = self.fixed_noises_dict[self.input_index]   
        else:
            keys = [k for k in inputs_dict.keys() if 'real_scale' in k]
            scales = len(keys)
            self.reals = [inputs_dict[f'real_scale{s}'] for s in range(scales)]
            self.reals_dict[self.input_index] = self.reals 
